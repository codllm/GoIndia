...
if (!document.getElementById("ch-styles")) {
  ...
  socket.on("ride-accepted", async (data) => {
    try {
      ...
      if (!ride || !ride.user?.socketId) return;

      io.to(ride.user.socketId).emit("ride-accepted", {
        rideId: ride._id,
        status: "accepted",
        caption: ride.captain,
        otp: ride.otp,
      });

      io.to(socket.id).emit("ride-accepted-success", {
        message: "Ride accepted, heading to pickup",
      });
    } catch (err) {
  ...
  style.textContent = `
    ...
    @keyframes ch-slide-left  { from{transform:translateX(0);opacity:1} to{transform:translateX(-120%);opacity:0} }
    @keyframes ch-slide-right { from{transform:translateX(0);opacity:1} to{transform:translateX(120%);opacity:0} }
    ...
    .ch-anim-fade  { animation: ch-fade 0.25s ease both }
    ...
    .ch-online-btn {
      ...
      transition: all 0.2s; box-shadow: 0 2px 12px rgba(0,0,0,0.12);
    }
    .ch-online-btn.online  { background: #05944F; color: #fff; box-shadow: 0 4px 16px rgba(5,148,79,0.35); }
    .ch-online-btn.offline { background: rgba(255,255,255,0.9); color: #555; }
    .ch-avatar-btn {
    ...
    /* ── Status sheet (waiting / offline) ── */
    .ch-status-sheet {
      position: absolute; bottom: 0; left: 0; right: 0;
      border-radius: 24px 24px 0 0;
      padding: 10px 22px 44px;
      z-index: 20;
      animation: ch-up 0.4s cubic-bezier(0.16,1,0.3,1) both;
    }
    ...
    .ch-ride-handle { width: 40px; height: 4px; border-radius: 2px; background: #DDD; margin: 0 auto 18px; }
    ...
    /* ── Ping dot ── */
    .ch-ping-wrap { position: relative; width: 8px; height: 8px; }
    .ch-ping-ring {
      position: absolute; inset: 0; border-radius: 50%;
      background: #05944F; animation: ch-ping 1.4s ease infinite;
    }
    ...
    .ch-route-line { width: 1.5px; height: 20px; background: #E8E8E8; margin: 4px 0 4px 4.25px; }
    .ch-route-tag  { font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #AAA; display: block; margin-bottom: 2px; }
    .ch-route-addr { font-size: 15px; font-weight: 600; color: #111; line-height: 1.35; display: block; }
    ...
    .ch-fare-label { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #AAA; margin-bottom: 3px; }
    .ch-fare-val   { font-family: 'JetBrains Mono', monospace; font-size: 24px; font-weight: 600; color: #111; }
    ...
    .ch-btn-dismiss {
      ...
      background: #F4F4F4; border: 1.5px solid #E8E8E8; cursor: pointer;
      font-family: 'Plus Jakarta Sans', sans-serif;
    ...
    /* ── Nav arrows ── */
    .ch-nav-arrows { display: flex; align-items: center; gap: 8px; }
    .ch-nav-btn {
      width: 32px; height: 32px; border-radius: 50%;
      background: #F4F4F4; border: 1px solid #E8E8E8; cursor: pointer;
...
const CaptionHome = () => {
  ...
  const [online, setOnline] = useState(
    localStorage.getItem("captionOnline") === "true"
  );
  ...
  useEffect(() => {
    ...
    if (online) {
      updateLocation();
      locationTimer.current = setInterval(updateLocation, 10000);
    }
  ...
  useEffect(() => {
    if (queueIdx >= rideQueue.length && rideQueue.length > 0) {
      setQueueIdx(rideQueue.length - 1);
    }
  }, [rideQueue]);

  const dismissRide = () => {
    setRideQueue((prev) => prev.filter((_, i) => i !== queueIdx));
    setQueueIdx((prev) => Math.max(0, prev - 1));
  };

  const goNext = () => {
    if (queueIdx < rideQueue.length - 1) {
      setCardAnim("ch-slide-in");
      setQueueIdx((p) => p + 1);
    }
  };
  const goPrev = () => {
    ...
    >
      ...
      <div className="ch-topbar">
        ...
        <div className="ch-topbar-right">
          <button
            className={`ch-online-btn ${online ? "online" : "offline"}`}
          ...
          >
            <img
              src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${caption.firstname}`}
              alt="profile"
      ...
      {/* ── STATUS SHEET (no rides) ── */}
      {!hasRides && (
        <div
          className="ch-status-sheet"
          style={{
            background: online ? "#FAFFF8" : "#FFF",
            border: "1px solid #E8E8E8",
          }}
        >
          <div
            className="ch-status-handle"
            style={{ background: online ? "#C8EDD8" : "#DDD" }}
          />
          ...
          {!online ? (
            <div style={{ textAlign: "center", paddingBottom: 8 }}>
              ...
              <p style={{ fontSize: 13, color: "#AAA" }}>
                Tap Online above to start receiving rides
              </p>
            </div>
          ) : (
            <div style={{ textAlign: "center", paddingBottom: 8 }}>
              ...
              >
                <div className="ch-ping-wrap" style={{ width: 8, height: 8 }}>
                  <div className="ch-ping-ring" />
                  <div className="ch-ping-core" />
                </div>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#05944F" }}>
      ...
      {hasRides && currentRide && (
        <div
          className={`ch-ride-card ${cardAnim}`}
          key={currentRide._id}
        ...
        >
          ...
          <div className="ch-queue-bar">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {/* Counter */}
              <span className="ch-queue-count">
                {queueIdx + 1} / {rideQueue.length}
              </span>
              ...
              {rideQueue.length > 1 && (
                <div className="ch-nav-arrows">
                  ...
                  <button
                    className="ch-nav-btn"
                    onClick={goNext}
                    disabled={queueIdx === rideQueue.length - 1}
                  >
          ...
          <div className="ch-route">
            <div className="ch-route-row">
              ...
              <div>
                <span className="ch-route-tag">Pickup</span>
                <span className="ch-route-addr">
                  {currentRide.pickup || "Pickup location"}
                </span>
              </div>
            </div>
            ...
            <div className="ch-route-row">
              ...
              <div>
                <span className="ch-route-tag">Drop-off</span>
                <span className="ch-route-addr">
          ...
          <div className="ch-actions">
            <button className="ch-btn-dismiss" onClick={dismissRide}>
              Skip
            </button>
            <button
              className="ch-btn-accept"
              onClick={acceptRide}
              disabled={accepting}
            >
              {accepting ? (
                <>
                  <span className="ch-spinner" /> Accepting…
                </>
              ) : (
                "Accept Ride →"
              )}
